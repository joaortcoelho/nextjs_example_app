...
const register = async (request: any, reply: FastifyReply) => {
  ...
  try {
    const existingUser = await main.getByParam<User>(
      "utilizador",
      "username",
      username
    );
...
const login = async (request: any, reply: FastifyReply) => {
  ...
  try {
    const user = await main.getByParam<User>(
      "utilizador",
      "username",
      username
    );
...
const profile = async (request: FastifyRequest, reply: FastifyReply) => {
  try {
    const decoded: any = request.server.jwt.verify(
      (request.headers as any).token
    );
    ...
    reply
      .status(500)
      .send({ message: "Erro ao carregar perfil do utilizador" });
  }
};
...
const authenticationHandler = (
  condition: (decoded: any, request: FastifyRequest) => boolean,
  errMsg: string
) => {
...
const authenticateTokenVerifier = authenticationHandler(
  (decoded: any) => true,
  "Token inválido"
);

const isUserAdmin = authenticationHandler(
  (decoded: any) => decoded.role === "admin",
  "Utilizador não é Admin"
);
