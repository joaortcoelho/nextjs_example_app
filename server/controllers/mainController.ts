...
// getByParam: get data from database table through param
const getByParam = async <T>(table: string, param: string, value: string): Promise<T | T[] | undefined> => {
  try {
    const results = await db.executeQuery<T[]>(`SELECT * FROM ${table} WHERE ${param} = ?`, [value]);
...
const updateInTable = async (table: string, id: number, data: object): Promise<void> => {
  const keys = Object.keys(data);
  const values = Object.values(data);
  const setString = keys.map((key) => `${key} = ?`).join(', ');

  return db.executeQuery<void>(`UPDATE ${table} SET ${setString} WHERE id = ?`, [...values, id]);
};
